...
different config file through the environment variable `OOD_CLUSTERS` or by
modifying the configuration in an initializer:

```ruby
# config/initializers/ood_appkit.rb

OodAppkit.configure do |config|
  config.clusters = OodAppkit::Cluster.all file: "/path/to/my/config"
end
```
